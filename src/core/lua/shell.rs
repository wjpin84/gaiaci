...
mod tests {
    ...
    fn test_shell_module_has_all_functions() {
        ...
        assert!(shell_mod.get::<mlua::Function>("arch").is_ok());
        assert!(shell_mod.get::<mlua::Function>("which").is_ok());
        assert!(shell_mod.get::<mlua::Function>("exists").is_ok());
    }

    #[test]
    fn test_shell_exists_function_with_commands() {
        let lua = Lua::new();
        let shell_mod = Shell::create(&lua).unwrap();
        let exists_fn: mlua::Function = shell_mod.get("exists").unwrap();
        
        // Test with a command that should exist on most systems
        let test_cmd = if cfg!(windows) { "cmd" } else { "sh" };
        let result: bool = exists_fn.call(test_cmd).unwrap();
        assert!(result, "Basic shell command should exist");
        
        // Test with a command that definitely doesn't exist
        let result: bool = exists_fn.call("definitely_nonexistent_command_12345").unwrap();
        assert!(!result, "Nonexistent command should return false");
    }

    #[test]
    fn test_shell_exists_function_with_absolute_paths() {
        let lua = Lua::new();
        let shell_mod = Shell::create(&lua).unwrap();
        let exists_fn: mlua::Function = shell_mod.get("exists").unwrap();
        
        // Test with the current executable (should exist)
        if let Ok(current_exe) = std::env::current_exe() {
            let result: bool = exists_fn.call(current_exe.to_string_lossy().to_string()).unwrap();
            assert!(result, "Current executable should exist");
        }
        
        // Test with a path that definitely doesn't exist
        let fake_path = if cfg!(windows) {
            "C:\\definitely\\nonexistent\\path\\executable.exe"
        } else {
            "/definitely/nonexistent/path/executable"
        };
        let result: bool = exists_fn.call(fake_path).unwrap();
        assert!(!result, "Nonexistent absolute path should return false");
    }

    #[test]
    fn test_shell_exists_vs_which_consistency() {
        let lua = Lua::new();
        let shell_mod = Shell::create(&lua).unwrap();
        let exists_fn: mlua::Function = shell_mod.get("exists").unwrap();
        let which_fn: mlua::Function = shell_mod.get("which").unwrap();
        ...
        let exists_result: bool = exists_fn.call(test_cmd).unwrap();
        let which_result: mlua::Result<String> = which_fn.call(test_cmd);
        
        // If exists returns true, which should succeed
        if exists_result {
            assert!(which_result.is_ok(), "If exists returns true, which should succeed");
        }
        
        // Test with nonexistent command
        let fake_cmd = "definitely_nonexistent_command_12345";
        let exists_result: bool = exists_fn.call(fake_cmd).unwrap();
        let which_result: mlua::Result<String> = which_fn.call(fake_cmd);
        
        // Both should indicate the command doesn't exist
        assert!(!exists_result, "Nonexistent command should return false from exists");
        assert!(which_result.is_err(), "Nonexistent command should fail from which");
    }
}